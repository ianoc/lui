...
case class GroupColumnIOBuilder(columnType: GroupType, index: Int) extends ColumnIOBuilder {
  private[this] var children = Vector[ColumnIOBuilder]()

  def add(child: ColumnIOBuilder): Unit = {
  ...
  override def setLevels(parentBox: LazyBox[GroupColumnIO],
    ...
    val gcIO = GroupColumnIO(columnType,
                             parentBox,
                             index,
                             repetitionLevel,
                             definitionLevel,
