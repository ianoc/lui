...
object ReadSupport {
   ...
   */
  def getSchemaForRead(fileMessageType: MessageType,
                       partialReadSchemaString: String): MessageType = {
    if (partialReadSchemaString == null)
      fileMessageType
    else {
      val requestedMessageType: MessageType =
        MessageTypeParser.parseMessageType(partialReadSchemaString)
      getSchemaForRead(fileMessageType, requestedMessageType)
    }
  }

  def getSchemaForRead(fileMessageType: MessageType,
                       projectedMessageType: MessageType): MessageType = {
...
abstract class ReadSupport[T] {
  ...
  private[lui] def setClassTag(ct: ClassTag[T]): Unit = {
    classTagHolder = ct
  }

  def getRecordReader(columns: PageReadStore,
