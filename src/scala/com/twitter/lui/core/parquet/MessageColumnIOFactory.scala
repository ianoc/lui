...
object MessageColumnIOFactory {
  ...
  def getRecordReader[T](columns: PageReadStore,
    ...
    readSupport.getRecordReader(columns,
                                configuration,
                                createdBy,
                                fileMetadata,
                                fileSchema,
  ...
  def buildMessageColumnIO(readContext: ReadContext,
                           fileSchema: MessageType,
                           createdBy: String,
    ...
    visitChildren(messageBuilder.rootColumnIOBuilder,
                  fileSchema,
                  readContext.requestedSchema.asGroupType)
  ...
  private[this] def visitNode(parentBuilder: GroupColumnIOBuilder,
                              ...
                              currentRequestedIndex: Int)(
      implicit strictTypeChecking: StrictTypeChecking,
      leafIndex: LeafIndex): Unit = {
    tpe match {
      ...
      case primitiveType: PrimitiveType =>
        ...
        val newIO =
          PrimitiveColumnIOBuilder(primitiveType, currentRequestedIndex, leafIndex.getAndIncr)
        parentBuilder.add(newIO)
    }

  }

  private[this] def visitChildren(groupIOBuilder: GroupColumnIOBuilder,
                                  groupType: GroupType,
