...
abstract class CacheColumnReader(writerVersion: ParsedVersion,
  ...
  def advanceSetRecord(newRecordPosition: Long): Unit = {
    if (newRecordPosition != cacheRecordPosition) {
      if (newRecordPosition < cacheRecordPosition) {
        sys.error(
          s"$columnName -> Attempted to access an old field, requested record $newRecordPosition, while the cache contains $cacheRecordPosition")
      }
  ...
  def skipInsideRecord(definitionLevel: Int,
                       previousPosition: Array[Int],
                       stateOffset: Array[Int],
                       targetOffset: Array[Int]): Boolean = {
    ...
    while (p < maxLen) {
      ...
      while (stateOffset(p) < targetOffset(p)) {
        ...
        if (cachePosition >= cacheSize) { // linter:ignore
          previousPosition(0) = cachePosition
          return false
        } else {
          cachePosition += 1
