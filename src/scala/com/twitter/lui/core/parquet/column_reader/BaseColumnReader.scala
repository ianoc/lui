...
abstract class BaseColumnReader(writerVersion: ParsedVersion,
                                path: ColumnDescriptor,
                                pageReader: PageReader,
                                val maxDefinitionLevel: Int,
                                val maxRepetitionLevel: Int)
    extends SpecificTypeGet {
  ...
  protected def isBehind(shouldBeAhead: Array[Int], shouldBeLessOrEqual: Array[Int]): Boolean = {
    val minLen =
      if (shouldBeLessOrEqual.length < shouldBeAhead.length) shouldBeLessOrEqual.length
      else shouldBeAhead.length
    var p = 0
    while (p < minLen) {
  ...
  var pageIdx = -1
  private[this] def readPageV1(page: DataPageV1): Unit = {
    try {
      ...
      require(
        repetitionLevelEncoding == Encoding.RLE || repetitionLevelEncoding == Encoding.BIT_PACKED,
        s"repetitionLevelEncoding encoding should be the Hybrid runlength bit packing decoder, was ${repetitionLevelEncoding.toString}")
      require(
        definitionLevelEncoding == Encoding.RLE || definitionLevelEncoding == Encoding.BIT_PACKED,
        s"definitionLevelEncoding encoding should be the Hybrid runlength bit packing decoder, was ${repetitionLevelEncoding.toString}")

      val r_bitWidth: Int = BytesUtils.getWidthFromMaxInt(
        EncodingAccessor.getMaxLevel(repetitionLevelEncoding, path, REPETITION_LEVEL))

      if (r_bitWidth > 0) {
        val r_in: ByteArrayInputStream =
        ...
        r_decoder =
          new RunLengthBitPackingHybridDecoder(r_bitWidth, bytes, offset, offset + r_length)
        offset += r_length
      } else {
        r_decoder = null
      }

      val d_bitWidth: Int = BytesUtils.getWidthFromMaxInt(
        EncodingAccessor.getMaxLevel(repetitionLevelEncoding, path, DEFINITION_LEVEL))

      if (d_bitWidth > 0) {
        val d_in: ByteArrayInputStream =
          new ByteArrayInputStream(bytes, offset, bytes.length - offset)
        val d_length: Int = BytesUtils.readIntLittleEndian(d_in)
        offset += 4; // 4 is for the length which is stored as 4 bytes little endian
        d_decoder =
          new RunLengthBitPackingHybridDecoder(d_bitWidth, bytes, offset, offset + d_length)
        offset += d_length
      } else {
        d_decoder = null
      }
  ...
  private[this] def initDataReader(dataEncoding: Encoding,
                                   bytes: Array[Byte],
                                   offset: Int,
                                   valueCount: Int): Unit = {
