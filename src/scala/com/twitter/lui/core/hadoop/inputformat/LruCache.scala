...
object LruCache {
  ...
  def apply[K, V <: LruCache.Value[K, V]](maxSize: Int)(implicit ev: Null <:< V) =
    new LruCache[K, V](maxSize, DEFAULT_LOAD_FACTOR, true)
...
final class LruCache[K, V <: LruCache.Value[K, V]](maxSize: Int,
                                                   loadFactor: Float,
                                                   accessOrder: Boolean)(implicit ev: Null <:< V) {
  import LruCache._
   ...
   */
  def put(key: K, newValue: V): Unit = {
    if (newValue == null || !newValue.isCurrent(key)) {
      if (Log.WARN) {
        LOG.warn(
          "Ignoring new cache entry for '" + key + "' because it is "
    ...
    if (oldValue != null && oldValue.isNewerThan(newValue)) {
      if (Log.WARN) {
        LOG.warn(
          "Ignoring new cache entry for '" + key + "' because "
