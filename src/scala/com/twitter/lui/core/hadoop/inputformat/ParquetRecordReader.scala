...
case class ParquetRecordReader[T](readSupport: ReadSupport[T]) extends RecordReader[Void, T] {
  ...
  override def initialize(inputSplit: InputSplit, context: TaskAttemptContext): Unit = {
    context match {
      case taskContext: TaskInputOutputContext[_, _, _, _] =>
        BenchmarkCounter.initCounterFromContext(taskContext)
      case _ =>
        LOG.error(
          "Can not initialize counter due to context is not a instance of TaskInputOutputContext, but is "
  ...
  def initialize(inputSplit: InputSplit, configuration: Configuration, reporter: Reporter): Unit = {
  ...
  private[this] def initializeInternalReader(split: ParquetInputSplit,
                                             configuration: Configuration): Unit = {
    val path: Path = split.getPath()
    val rowGroupOffsets: Array[Long] = split.getRowGroupOffsets
    // if task.side.metadata is set, rowGroupOffsets is null
    val (blocks: IndexedSeq[BlockMetaData], footer: ParquetMetadata) =
      if (rowGroupOffsets == null) {
        ...
        val blocks =
          footer
            .getBlocks()
            .asScala
            .filter { block: BlockMetaData =>
              offsets.contains(block.getStartingPos())
            }
            .toIndexedSeq
    ...
    internalReader = InternalParquetRecordReader(readSupport,
                                                 fileSchema,
  ...
  private[this] def checkDeltaByteArrayProblem(meta: FileMetaData,
                                               conf: Configuration,
                                               block: BlockMetaData): Unit = {
    // splitting files?
