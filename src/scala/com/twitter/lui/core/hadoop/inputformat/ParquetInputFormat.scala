...
object ParquetInputFormat {
  ...
  def setTaskSideMetaData(job: Job, taskSideMetadata: Boolean): Unit = {
  ...
  def setReadSupportClass(conf: JobConf, readSupportClass: Class[_], targetClass: Class[_]): Unit = {
    conf.set(READ_SUPPORT_CLASS, readSupportClass.getName)
    conf.set(TARGET_CLASS, targetClass.getName)
  }

  def getClassFromConfig(configuration: Configuration,
                         configName: String,
                         assignableFromOpt: Option[Class[_]]): (String, Class[_]) = {
    ...
    try {
      ...
      assignableFromOpt.foreach { assignableFrom =>
        if (!assignableFrom.isAssignableFrom(foundClass)) {
          throw new BadConfigurationException(
            "class " + className + " set in job conf at "
      ...
      case e: ClassNotFoundException =>
        throw new BadConfigurationException(
          "could not instantiate class " + className + " set in job conf at " + configName,
          e);
    }
  }
  ...
  def getReadSupportClass[T](configuration: Configuration): (String, Class[_ <: ReadSupport[T]]) = {
    val (name, clazz) =
      getClassFromConfig(configuration, READ_SUPPORT_CLASS, Some(classOf[ReadSupport[_]]))

    (name, clazz.asInstanceOf[Class[ReadSupport[T]]])
  }

  def getTargetClass(configuration: Configuration): (String, Class[Any]) = {
    val (name, clazz) = getClassFromConfig(configuration, TARGET_CLASS, None)

    (name, clazz.asInstanceOf[Class[Any]])
  }
  ...
  def getReadSupportInstance[T](configuration: Configuration): ReadSupport[T] = {
    ...
    try {
      ...
      val (targetClassName, clazz: Class[Any]) = getTargetClass(configuration)
      ...
      case e: InstantiationException =>
        throw new BadConfigurationException(
          s"could not instantiate read support class: $className",
          e)
  ...
  private def getAllFileRecursively(files: JList[FileStatus],
                                    conf: Configuration): JList[FileStatus] = {
  ...
  object FootersCacheValue {
    def apply(status: FileStatusWrapper, footer: Footer): FootersCacheValue =
      new FootersCacheValue(status.getModificationTime,
                            new Footer(footer.getFile, footer.getParquetMetadata))
  }
  class FootersCacheValue(val modificationTime: Long, val footer: Footer)
      extends LruCache.Value[FileStatusWrapper, FootersCacheValue] {
    ...
    override def isCurrent(key: FileStatusWrapper): Boolean = {
      ...
      if (Log.DEBUG && !isCurrent) {
        LOG.debug(
          "The cache value for '" + key + "' is not current: "
...
class ParquetInputFormat[T] extends FileInputFormat[Void, T] {
  ...
  def getSplits(configuration: Configuration, footers: JList[Footer]): JList[ParquetInputSplit] = {
    ...
    if (maxSplitSize < 0 || minSplitSize < 0) {
      throw new ParquetDecodingException(
        "maxSplitSize or minSplitSize should not be negative: maxSplitSize = " + maxSplitSize + "; minSplitSize = " + minSplitSize);
    }
    val globalMetaData: GlobalMetaData = org.apache.parquet.hadoop.ParquetFileWriterBridge
      .getGlobalMetaData(footers, strictTypeChecking)
    val readContext: ReadContext = getReadSupportInstance(configuration).init(
      new InitContext(configuration,
                      globalMetaData.getKeyValueMetaData(),
                      globalMetaData.getSchema()))

    new ClientSideMetadataSplitStrategy()
  ...
  def getFooters(jobContext: JobContext): JList[Footer] = {
    ...
    if (footersCache == null) {
      footersCache = LruCache[FileStatusWrapper, FootersCacheValue](
    ...
    statuses.asScala.foreach { status =>
      ...
      if (Log.DEBUG) {
        LOG.debug(
          s"""Cache entry ${if (cacheEntry == null) "not" else ""} found for '${status.getPath}'""")
      }
      if (cacheEntry != null) {
    ...
    if (Log.DEBUG) {
      LOG.debug(
        "found " + footers.size() + " footers in cache and adding up "
  ...
  def getFooters(configuration: Configuration, statuses: Collection[FileStatus]): JList[Footer] = {
    ...
    org.apache.parquet.hadoop.ParquetFileReader
      .readAllFootersInParallelUsingSummaryFiles(configuration, statuses, taskSideMetaData)
  }
