...
case class ColumnChunkPreStaged(descriptor: ChunkDescriptor,
                                f: FSDataInputStream,
...
case class ColumnChunkData(var pagesInChunk: List[DataPage],
  ...
  def popDataPage: DataPage =
    if (pagesInChunk.isEmpty) null
    else {
...
object ColumnChunkData {
  ...
  def apply(preStaged: ColumnChunkPreStaged): ColumnChunkData = {
    ...
    val totalSize = mc.getTotalSize.toInt
    ...
    val decompressor: BytesDecompressor =
      preStaged.codecFactory.getDecompressor(descriptor.metadata.getCodec)
        ...
        case PageType.DICTIONARY_PAGE =>
          ...
          if (dictionaryPage != null) {
            throw new ParquetDecodingException(
              s"more than one dictionary page in column : ${descriptor.col}")
          }
          val dicHeader: DictionaryPageHeader = pageHeader.getDictionary_page_header
          dictionaryPage = new DictionaryPage(bais.readAsBytesInput(compressedPageSize),
    ...
    if (valuesCountReadSoFar != descriptor.metadata.getValueCount) {
      throw new IOException(
        s"""Expected ${descriptor.metadata.getValueCount} values in column chunk at
            ...
            | pages ending at file offset ${descriptor.fileOffset + bais.getPos}"""
          .stripMargin('|'))
    }
    ColumnChunkData(pagesInChunk, dictionaryPage, decompressor)
  }
}

private[inputformat] class ColumnChunkPageReader(prestagedData: ColumnChunkPreStaged)
    extends PageReader {
    ...
    if (columnChunkData.pagesInChunk.isEmpty) {
      ...
      compressedPage match {
        case dataPageV1: DataPageV1 =>
          try {
            new DataPageV1(columnChunkData.decompressor
                             .decompress(dataPageV1.getBytes(), dataPageV1.getUncompressedSize()),
  ...
  override def readDictionaryPage(): DictionaryPage =
    if (columnChunkData.dictionaryPage == null) {
      ...
      try {
        ...
        new DictionaryPage(
          columnChunkData.decompressor.decompress(compressedDictionaryPage.getBytes(),
