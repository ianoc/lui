...
object MapRedParquetInputFormat {
  ...
  private[MapRedParquetInputFormat] class ParquetInputSplitWrapper(
      var realSplit: ParquetInputSplit)
      extends InputSplit {
  ...
  private[MapRedParquetInputFormat] object RecordReaderWrapper {
    def apply[V](oldSplit: InputSplit,
                 oldJobConf: JobConf,
                 reporter: Reporter): RecordReaderWrapper[V] =
      try {
        val realReader =
          new ParquetRecordReader[V](ParquetInputFormat.getReadSupportInstance[V](oldJobConf))

        oldSplit match {
          case pisq: ParquetInputSplitWrapper =>
            realReader.initialize(pisq.realSplit, oldJobConf, reporter)
        ...
        RecordReaderWrapper[V](realReader, oldSplit.getLength, valueContainer, firstRecord, eof)
...
class MapRedParquetInputFormat[V]
    extends org.apache.hadoop.mapred.FileInputFormat[Void, Container[V]] {
