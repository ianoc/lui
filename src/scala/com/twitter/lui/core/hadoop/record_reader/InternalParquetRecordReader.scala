...
import com.twitter.lui.{RecordReader, MessageColumnIOFactory}
...
object InternalParquetRecordReader {
  ...
  def apply[T](readSupport: ReadSupport[T],
    ...
    val reader = new ParquetFileReader(configuration,
                                       parquetFileMetadata,
                                       file,
                                       blocks,
                                       columnsToReadFromDisk)
    ...
    InternalParquetRecordReader[T](readSupport,
                                   file,
...
case class InternalParquetRecordReader[T] private (
  ...
  private[this] def checkRead(): Unit = {
    if (current == totalCountLoadedSoFar) {
      if (current != 0) {
        totalTimeSpentProcessingRecords += (System
          .currentTimeMillis() - startedAssemblingCurrentBlockAt)
        if (Log.INFO) {
          LOG.info(
            s"Assembled and processed $totalCountLoadedSoFar records from $columnCount columns in $totalTimeSpentProcessingRecords  ms: ${totalCountLoadedSoFar.toFloat / totalTimeSpentProcessingRecords}  rec/ms, ${(totalCountLoadedSoFar.toFloat * columnCount / totalTimeSpentProcessingRecords)} cell/ms")
          ...
          if (totalTime != 0) {
            ...
            LOG.info(
              s"time spent so far $percentReading % reading ($totalTimeSpentReadingBytes ms) and $percentProcessing % processing ($totalTimeSpentProcessingRecords ms)")
          }
        }
      }
      ...
      if (pages == null) {
        throw new IOException(
          s"expecting more rows but reached last block. Read $current out of $total")
      }
      ...
      if (Log.INFO)
        LOG.info(
          "block read in memory in " + timeSpentReading + " ms. row count = " + pages
            .getRowCount())

      recordReader = MessageColumnIOFactory.getRecordReader[T](pages,
                                                               configuration,
                                                               createdBy,
      ...
      try {
        ...
        case e: RuntimeException =>
          throw new ParquetDecodingException(
            s"Can not read value at $current in block $currentBlock in file $file",
            e)
      }
    }
    recordFound
