...
object LuiScroogeReadSupport {
  ...
  def getGenerator(className: String,
                   thriftColumns: Map[String,
                                      (ParquetFieldInfo,
                                       Option[Int],
                                       Option[ParquetThriftEnumInfo],
                                       Vector[Short])]): ScroogeGenerator =
    cache.synchronized {
...
class LuiScroogeReadSupport[T >: Null <: ThriftStruct] extends ReadSupport[T] {
  ...
  override def getRecordReader(columns: PageReadStore,
    ...
    val messageColumnIO = MessageColumnIOFactory
      .buildMessageColumnIO(readContext, fileSchema, createdBy, strictTypeChecking)
    ...
    val thriftIdPathToParquetIndex: ParquetMappingToThriftMapping =
      ThriftIdPathToParquetIndex.get(fileSchema, thriftMetaData.getDescriptor)
    require(thriftIdPathToParquetIndex.m
              .map(_._1)
              .size == thriftIdPathToParquetIndex.m.map(_._1).distinct.size,
            "Thrift id's should be distinct!")

    val parquetFileThriftEnumMetadata: Map[ThriftIdPath, ParquetThriftEnumInfo] =
      ThriftEnumMetadata.get(thriftMetaData.getDescriptor).map {
        case (k, v) => k -> ParquetThriftEnumInfo(v)
      }
    ...
    val thriftIntermediateData: Seq[(ThriftIdPath, ParquetFieldInfo)] =
      thriftIdPathToParquetIndex.m.toSeq.map {
        case (tidPath: ThriftIdPath, pfi: ParquetFieldInfo) =>
          ...
          val enrichedPfi =
            pfi.copy(maxDefinitionLevel = r.maxDefLvl, maxRepetitionLevel = r.maxRepLvl)
    ...
    val primitiveSet: List[((ThriftIdPath, BaseColumnReader), Int)] =
      thriftIntermediateData.toIterator.filter { case (_, pfi) => pfi.parquetSchema.isPrimitive }.map {
        case (tidPath, pfi) =>
          val path = pfi.strPath
          val r = mapping(path.toList)

          val pType = pfi.parquetSchema.asPrimitiveType
          val columnDescriptor = new ColumnDescriptor(path.toArray,
          ...
          val col = (pfi.thriftType, reUsedLut(tidPath) > 1) match {
            ...
            case (_: ThriftType.StringType, true) =>
              cache.StringColumnReader(writerVersion,
                                       columnDescriptor,
                                       pageReader,
                                       r.maxDefLvl,
                                       r.maxRepLvl)
            case (_: ThriftType.BoolType, true) =>
              cache.BooleanColumnReader(writerVersion,
                                        columnDescriptor,
                                        pageReader,
                                        r.maxDefLvl,
                                        r.maxRepLvl)
            case (_: ThriftType.ByteType, true) =>
              cache.IntColumnReader(writerVersion,
                                    columnDescriptor,
                                    pageReader,
                                    r.maxDefLvl,
                                    r.maxRepLvl)
            case (_: ThriftType.I32Type, true) =>
              cache.IntColumnReader(writerVersion,
                                    columnDescriptor,
                                    pageReader,
                                    r.maxDefLvl,
                                    r.maxRepLvl)
            case (_: ThriftType.I16Type, true) =>
              cache.IntColumnReader(writerVersion,
                                    columnDescriptor,
                                    pageReader,
                                    r.maxDefLvl,
                                    r.maxRepLvl)
            case (_: ThriftType.I64Type, true) =>
              cache.LongColumnReader(writerVersion,
                                     columnDescriptor,
                                     pageReader,
                                     r.maxDefLvl,
                                     r.maxRepLvl)
            case (_: ThriftType.DoubleType, true) =>
              cache.DoubleColumnReader(writerVersion,
                                       columnDescriptor,
                                       pageReader,
                                       r.maxDefLvl,
                                       r.maxRepLvl)
            case (_: ThriftType.EnumType, true) =>
              cache.StringColumnReader(writerVersion,
                                       columnDescriptor,
                                       pageReader,
                                       r.maxDefLvl,
            ...
            case (_: ThriftType.StringType, false) =>
              noncache.StringColumnReader(writerVersion,
                                          columnDescriptor,
                                          pageReader,
                                          r.maxDefLvl,
                                          r.maxRepLvl)
            case (_: ThriftType.BoolType, false) =>
              noncache.BooleanColumnReader(writerVersion,
                                           columnDescriptor,
                                           pageReader,
                                           r.maxDefLvl,
                                           r.maxRepLvl)
            case (_: ThriftType.ByteType, false) =>
              noncache.IntColumnReader(writerVersion,
                                       columnDescriptor,
                                       pageReader,
                                       r.maxDefLvl,
                                       r.maxRepLvl)
            case (_: ThriftType.I32Type, false) =>
              noncache.IntColumnReader(writerVersion,
                                       columnDescriptor,
                                       pageReader,
                                       r.maxDefLvl,
                                       r.maxRepLvl)
            case (_: ThriftType.I16Type, false) =>
              noncache.IntColumnReader(writerVersion,
                                       columnDescriptor,
                                       pageReader,
                                       r.maxDefLvl,
                                       r.maxRepLvl)
            case (_: ThriftType.I64Type, false) =>
              noncache.LongColumnReader(writerVersion,
                                        columnDescriptor,
                                        pageReader,
                                        r.maxDefLvl,
                                        r.maxRepLvl)
            case (_: ThriftType.DoubleType, false) =>
              noncache.DoubleColumnReader(writerVersion,
                                          columnDescriptor,
                                          pageReader,
                                          r.maxDefLvl,
                                          r.maxRepLvl)
            case (_: ThriftType.EnumType, false) =>
              noncache.StringColumnReader(writerVersion,
                                          columnDescriptor,
                                          pageReader,
                                          r.maxDefLvl,
            ...
            case _ =>
              sys.error(
                s"Missing column reader for : ${pfi.thriftType.getClass} -> ${pfi.thriftType}")
          }
          (tidPath, col)
      }.zipWithIndex.toList

    val allColData: Array[BaseColumnReader] = primitiveSet.map(_._1._2).toArray
    val primitiveLut: Map[ThriftIdPath, Int] = primitiveSet.map {
      case ((path, _), idx) => path -> idx
    }.toMap

    val thriftColumns: Map[String,
                           (ParquetFieldInfo,
                            Option[Int],
                            Option[ParquetThriftEnumInfo],
                            Vector[Short])] =
      thriftIntermediateData.map {
        case (tidPath, pfi) =>
          (tidPath.toVector.mkString("/"),
           (pfi,
            primitiveLut.get(tidPath),
            parquetFileThriftEnumMetadata.get(tidPath),
            tidPath.toVector))
      }.toMap
  ...
  override def init(context: InitContext): ReadContext =
    ReadContext(context.getFileSchema) // no projection yet
}
