...
object ThriftEnumMetadata
    extends StateVisitor[Map[ThriftIdPath, Map[String, Int]], Vector[Short]] {

  def get(fileDescriptor: StructType) = fileDescriptor.accept(this, Vector())

  override def visit(enumType: EnumType,
                     state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] = {
    val m = enumType.getValues
      .iterator()
      .asScala
      .map { e =>
        (e.getName, e.getId)
  ...
  override def visit(listType: ListType,
                     state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] =
  ...
  override def visit(boolType: BoolType,
                     state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] = Map()
  override def visit(byteType: ByteType,
                     state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] = Map()
  override def visit(doubleType: DoubleType,
                     state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] = Map()
  override def visit(i16Type: I16Type, state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] =
    Map()
  override def visit(i32Type: I32Type, state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] =
    Map()
  override def visit(i64Type: I64Type, state: Vector[Short]): Map[ThriftIdPath, Map[String, Int]] =
    Map()
