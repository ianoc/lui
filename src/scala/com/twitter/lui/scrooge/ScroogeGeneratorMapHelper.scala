...
object ScroogeGeneratorMapHelper {
  ...
  def clazzToTpeAndColumn(clazzF: Class[_],
                          colName: String,
                          fieldIdx: Int,
                          innerIdx: Int,
                          path: Vector[Short]) =
    clazzF.getName match {
      case "java.lang.String" =>
        ("String",
         raw"{$colName.primitiveAdvance(recordIdx, repetitionOffsets); $colName.getString()}",
         true)
      case "boolean" =>
        ("Boolean",
         raw"{$colName.primitiveAdvance(recordIdx, repetitionOffsets); $colName.getBoolean()}",
         true)
      case "byte" =>
        ("Byte",
         raw"{$colName.primitiveAdvance(recordIdx, repetitionOffsets); $colName.getInteger().toByte}",
         true)
      case "double" =>
        ("Double",
         raw"{$colName.primitiveAdvance(recordIdx, repetitionOffsets); $colName.getDouble()}",
         true)
      case "int" =>
        ("Int",
         raw"{$colName.primitiveAdvance(recordIdx, repetitionOffsets); $colName.getInteger()}",
         true)
      case "long" =>
        ("Long",
         raw"{$colName.primitiveAdvance(recordIdx, repetitionOffsets); $colName.getLong()}",
         true)
      case "short" =>
        ("Short",
         raw"{$colName.primitiveAdvance(recordIdx, repetitionOffsets); $colName.getInteger().toShort}",
         true)
      case name if classOf[ThriftEnum].isAssignableFrom(clazzF) =>
        (name,
         raw"""{$colName.primitiveAdvance(recordIdx, repetitionOffsets); enumDecode${fieldIdx}_${innerIdx}($colName.getString())}""",
         true)
      case name if classOf[ThriftUnion].isAssignableFrom(clazzF) =>
        (name,
         raw"""scroogeGenerator.buildStruct(classOf[$name], $name.fieldInfos, "${path
           .mkString("/")}", colReaders, recordIdx, repetitionOffsets).asInstanceOf[$name]""",
         false)
  ...
  def apply(
    ...
    val keyClazz = keyManifest.runtimeClass
    val valueClazz = valueManifest.runtimeClass
    ...
    val (_, Some(fieldClosestPrimitiveColumnIdx), _, _) = locFieldInfo.closestPrimitiveChild.map {
      pri =>
        mapData(pri.toVector.mkString("/"))
    }.getOrElse(mapData(path))

    val (keySubTpeName, keyReadColumn, keyIsPrimitive) =
      clazzToTpeAndColumn(keyClazz, "keyColumn", fieldIdx, 0, pathK)
    val (valueSubTpeName, valueReadColumn, valueIsPrimitive) =
      clazzToTpeAndColumn(valueClazz, "valueColumn", fieldIdx, 1, pathV)

    val fieldTypeString =
      if (f.isOptional) s"Option[Map[$keySubTpeName, $valueSubTpeName]]"
      else s"Map[$keySubTpeName, $valueSubTpeName]"
    ...
    ("",
     raw"""
          lazy val `${f.name}`: $fieldTypeString = {
              ...
              val stateOffests = Array[Int](${(0 until fieldRepetitionLevel + 2)
       .map(_ => 0)
       .mkString(",")})
              var elements: Int = 0
