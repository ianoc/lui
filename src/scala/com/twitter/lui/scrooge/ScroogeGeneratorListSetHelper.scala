...
object ScroogeGeneratorListSetHelper {
  ...
  def nonMethod(
    ...
    val (_, Some(fieldClosestPrimitiveColumnIdx), _, _) =
      repeatedGroupFieldInfo.closestPrimitiveChild.map { pri =>
        mapData(pri.toVector.mkString("/"))
      }.getOrElse(mapData(s"$path/1"))

    val (subTpeName, readColumn) = clazzF.getName match {
      case "java.nio.ByteBuffer" =>
        ("java.nio.ByteBuffer",
         "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getBinary()}")
      case "java.lang.String" =>
        ("String",
         "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getString()}")
      case "boolean" =>
        ("Boolean",
         "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getBoolean()}")
      case "byte" =>
        ("Byte",
         "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getInteger().toByte}")
      case "double" =>
        ("Double",
         "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getDouble()}")
      case "int" =>
        ("Int",
         "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getInteger()}")
      case "long" =>
        ("Long", "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getLong()}")
      case "short" =>
        ("Short",
         "{primitive.primitiveAdvance(recordIdx, repetitionOffsets); primitive.getInteger().toShort}")
      case "scala.collection.Seq" =>
        ...
        (s"scala.collection.Seq[$innerClassName]",
         ScroogeGeneratorListSetHelper
           .nonMethod(manifestF,
                      ExistingPath(repeatedGroupPathV.mkString("/")),
                      cols,
                      mapData,
                      false,
                      true)
      ...
      case name if classOf[ThriftEnum].isAssignableFrom(clazzF) =>
        (name,
         s"""{primitive.primitiveAdvance(recordIdx, repetitionOffsets); ${name}enumDecode(primitive.getString())}""")
      case name if classOf[ThriftUnion].isAssignableFrom(clazzF) =>
        (name,
         s"""scroogeGenerator.buildUnion(classOf[$name], $name.fieldInfos, "${repeatedGroupPathV
      ...
      case name if classOf[ThriftStruct].isAssignableFrom(clazzF) =>
        (name,
         s"""scroogeGenerator.buildStruct(classOf[$name], $name.fieldInfos, "${repeatedGroupPathV
           .mkString("/")}", colReaders, recordIdx, repetitionOffsets).asInstanceOf[$name]""")
    }
...
if(elements > 0)
  listBuilder.result()
    ...
    if(primitive.notNull(${optionalGroupFieldInfo.maxDefinitionLevel}, recordIdx,cachePos,stateOffests,repetitionOffsets))
      ...
      case (_, true) => "listBuilder.result()"
    }
    ...
    ("",
     raw"""{
              ...
              val stateOffests = Array[Int](${(0 until maxRepLevelOfPrimitive + 1)
       .map(_ => 0)
       .mkString(",")})
              var elements: Int = 0
  ...
  def apply(f: ThriftStructFieldInfo,
    ...
    val (objectSettings, methodContents) =
      nonMethod(f.manifest, path, cols, mapData, f.isOptional, f.isRequired)
    ...
    val fieldTypeString =
      if (f.isOptional) s"Option[${returnType}[$subTpeName]]" else s"${returnType}[$subTpeName]"
