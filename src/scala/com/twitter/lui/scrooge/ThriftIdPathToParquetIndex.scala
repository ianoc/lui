...
object ParquetMappingToThriftMappingMonoid extends Monoid[ParquetMappingToThriftMapping] {
  ...
  private[this] def exceptionSemigroup[T] = new Semigroup[T] {
    def plus(a: T, b: T) =
      sys.error(s"Fields $a and $b should not be combined. ExceptionSemigroup!.")
  }

  def plus(a: ParquetMappingToThriftMapping,
           b: ParquetMappingToThriftMapping): ParquetMappingToThriftMapping = {
 ...
 */
object ThriftIdPathToParquetIndex
    extends StateVisitor[ParquetMappingToThriftMapping, PathToSchemaType] {
  implicit val pmtmMonoid = ParquetMappingToThriftMappingMonoid
  def get(fileSchema: MessageType, fileDescriptor: StructType) = {
    val recursive = fileDescriptor
      .accept(this, PathToSchemaType(Vector(), Vector(), Vector(), fileSchema, false))

    val primitiveChild = Some(recursive.m.iterator.filter(_._2.isPrimitive).next._1)

    Monoid.plus(ParquetMappingToThriftMapping(
                  List(
                    (ThriftIdPath(Vector()),
                     ParquetFieldInfo(Vector(),
                                      Vector(),
                                      fileSchema,
                                      primitiveChild,
                                      0,
  ...
  override def visit(structType: StructType,
                     state: PathToSchemaType): ParquetMappingToThriftMapping = {
    ...
    Monoid.sum(structType.getChildren.iterator.asScala.flatMap { c: ThriftField =>
      ...
      val childPathToSchemaType = PathToSchemaType(state.thriftIdPath :+ c.getFieldId,
                                                   parquetIdPath,
                                                   parquetStringPath,
                                                   matchingChild,
      ...
      val primitiveChild =
        if (matchingChild.isPrimitive) None
        else Some(childsMaps.m.iterator.filter(_._2.isPrimitive).next._1)

      val childNodeValue = ParquetMappingToThriftMapping(
        List(
          ThriftIdPath(childPathToSchemaType.thriftIdPath) -> ParquetFieldInfo(
            parquetIdPath,
            parquetStringPath,
            matchingChild,
            primitiveChild,
  ...
  override def visit(mapType: MapType, state: PathToSchemaType): ParquetMappingToThriftMapping = {
    ...
    Monoid.sum(List(keyT, valueT).flatMap {
      case (parquetCType, pathExtension, thriftInnerType) =>
        val parquetStringPath: Vector[String] = state.parquetStringPath ++ Vector(
            "map",
            parquetCType.getName)
        val parquetIdPath: Vector[Int] = state.parquetIdPath :+ pathExtension

        val childPathToSchemaType = PathToSchemaType(state.thriftIdPath :+ pathExtension.toShort,
                                                     parquetIdPath,
                                                     parquetStringPath,
                                                     parquetCType,
                                                     true)
        ...
        val primitiveChild =
          if (parquetCType.isPrimitive) None
          else Some(childsMaps.m.iterator.filter(_._2.isPrimitive).next._1)

        val childNodeValue = ParquetMappingToThriftMapping(
          List(
            ThriftIdPath(childPathToSchemaType.thriftIdPath) -> ParquetFieldInfo(parquetIdPath,
                                                                                 parquetStringPath,
                                                                                 parquetCType,
                                                                                 primitiveChild,
                                                                                 0,
  ...
  override def visit(setType: SetType, state: PathToSchemaType): ParquetMappingToThriftMapping = {
    ...
    Monoid.sum(List(c).flatMap { c: ThriftType =>
      ...
      val childPathToSchemaType = PathToSchemaType(state.thriftIdPath :+ 1.toShort,
                                                   parquetIdPath,
                                                   parquetStringPath,
                                                   parquetCType,
                                                   true)
  ...
  override def visit(enumType: EnumType, state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero

  override def visit(boolType: BoolType, state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero

  override def visit(byteType: ByteType, state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero

  override def visit(doubleType: DoubleType,
                     state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero

  override def visit(i16Type: I16Type, state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero

  override def visit(i32Type: I32Type, state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero

  override def visit(i64Type: I64Type, state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero

  override def visit(stringType: StringType,
                     state: PathToSchemaType): ParquetMappingToThriftMapping =
    ParquetMappingToThriftMappingMonoid.zero
