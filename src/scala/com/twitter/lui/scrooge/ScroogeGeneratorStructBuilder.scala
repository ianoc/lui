...
object ScroogeGeneratorStructBuilder {
  ...
  private[scrooge] def apply[T <: ThriftStruct](
      ...
      cols: Array[BaseColumnReader])(implicit compiler: CustomCompiler)
    ...
    val (closestPrimitiveFieldInfo, Some(closestPrimitiveColumnIdx), optiEnumInfo, _) = mapData(
      structFieldInfo.closestPrimitiveChild.get.toVector.mkString("/"))

    val repetitionLevel = structFieldInfo.maxRepetitionLevel

    val enumHelpers = ScroogeGeneratorEnumHelper(metadata, existingPath, mapData)
    ...
    val (objectSections: String, fieldSections: String) = Monoid.sum(metadata.zipWithIndex.map {
      case (f: ThriftStructFieldInfo, idx) =>
        val (objectCode: String, builderStr: String) = f.toGenT match {
          case PrimitiveT => ScroogeGeneratorPrimitiveHelper(f, existingPath, mapData)

          case StructT => ScroogeGeneratorStructHelper(f, existingPath, mapData)

          case ListT => ScroogeGeneratorListSetHelper(f, existingPath, cols, mapData)

          case SetT => ScroogeGeneratorListSetHelper(f, existingPath, cols, mapData)

          case MapT => ScroogeGeneratorMapHelper(f, existingPath, mapData, idx)
        }
    ...
    val repetitionCopy =
      if (structFieldInfo.maxRepetitionLevel > 0)
