...
object ScroogeGeneratorUnionBuilder {
  ...
  private[scrooge] def apply[T <: ThriftStruct](
      mapData: Map[String,
                   (ParquetFieldInfo, Option[Int], Option[ParquetThriftEnumInfo], Vector[Short])],
      mainClass: Class[T],
      metadata: List[ThriftUnionFieldInfo[_, _]],
      existingPath: ExistingPath)(implicit compiler: CustomCompiler)
    : ((Array[BaseColumnReader], ScroogeGenerator, Long, Array[Int]) => T) = {
    ...
    val (objectSections: String, fieldSections: String) = Monoid.sum(metadata.zipWithIndex.map {
      case (structUnionFieldInfo: ThriftUnionFieldInfo[_, _], idx) =>
        ...
        val (objectCode: String, builderStr: String) = structFieldInfo.toGenT match {
          case PrimitiveT =>
            ScroogeGeneratorPrimitiveHelper.nonMethod(structFieldInfo, existingPath, mapData)

          case StructT =>
            ScroogeGeneratorStructHelper.nonMethod(structFieldInfo, existingPath, mapData)

          case t => sys.error(s"Not expected/implemented for type $t")
        ...
        val path =
          if (existingPath.toStr.isEmpty) structFieldInfo.id.toString
          else s"${existingPath.toStr}/${structFieldInfo.id}"
        val (pfi, optionPrimitiveCol, optParquetThriftEnumInfo, closestPrimitiveChild) =
          mapData(path)
        val primitiveColumnIndex = optionPrimitiveCol.getOrElse(
          mapData(pfi.closestPrimitiveChild.get.toVector.mkString("/"))._2.get)

        val fieldNotNullTest =
          if (pfi.maxRepetitionLevel > 0)
        ...
        val classInstance =
          structUnionFieldInfo.fieldClassTag.runtimeClass.getName.replaceAll("\\$", ".")
...
new Function4[Array[BaseColumnReader], ScroogeGenerator, Long,  Array[Int], $className]{
  def apply(colReaders: Array[BaseColumnReader], scroogeGenerator: ScroogeGenerator , recordIdx: Long, repetitionOffsets: Array[Int]): $className = {
    ...
    try {
      compiler
        .compile[Function4[Array[BaseColumnReader], ScroogeGenerator, Long, Array[Int], T]](code)
    } catch {
      case e: Throwable =>
