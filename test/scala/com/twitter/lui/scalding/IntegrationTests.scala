  ...
  def recursiveCompare(unknownA: Any, unknownB: Any): Unit =
    (unknownA, unknownB) match {
      case (a: Product, b: Product) =>
        Try(productCompare(a, b)) match {
          case Success(_) => ()
          case Failure(e) => throw new Exception(s"Comparing type: ${a.getClass}\n${e.getMessage}")
        }
      case (a: TraversableOnce[Any], b: TraversableOnce[Any]) =>
        Try(traversableOnceCompare(a, b)) match {
          case Success(_) => ()
          case Failure(e) => throw new Exception(s"Comparing type: ${a.getClass}\n${e.getMessage}")
        }
      case (a, b) =>
        if (a != b)
          sys.error(s"Reference $a was not equal to test target $b, of type ${a.getClass}")
    }
}
...
class IntegrationTests extends WordSpec {
  ...
  org.apache.log4j.Logger
    .getLogger("org.apache.parquet.hadoop.ParquetOutputFormat")
    .setLevel(org.apache.log4j.Level.ERROR)
  org.apache.log4j.Logger
    .getLogger("org.apache.parquet.hadoop.codec.CodecConfig")
    .setLevel(org.apache.log4j.Level.ERROR)
  org.apache.log4j.Logger
    .getLogger("org.apache.parquet.hadoop.ColumnChunkPageWriteStore")
    .setLevel(org.apache.log4j.Level.ERROR)
  org.apache.log4j.Logger
    .getLogger("org.apache.parquet.hadoop.thrift.AbstractThriftWriteSupport")
    .setLevel(org.apache.log4j.Level.ERROR)
  org.apache.log4j.Logger
    .getLogger("org.apache.parquet.hadoop.ParquetFileReader")
    .setLevel(org.apache.log4j.Level.ERROR)
  org.apache.log4j.Logger
    .getLogger("org.apache.parquet.Log")
    .setLevel(org.apache.log4j.Level.ERROR)
  ...
  "StringAndBinary Source should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[StringAndBinary]("/tmp/scalding_StringAndBinary")
    val readTarget = new FPLuiScroogeSource[StringAndBinary]("/tmp/scalding_StringAndBinary")
  ...
  "RequiredPrimitiveFixture Source should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[RequiredPrimitiveFixture](
      "/tmp/scalding_RequiredPrimitiveFixture")
    val readTarget =
      new FPLuiScroogeSource[RequiredPrimitiveFixture]("/tmp/scalding_RequiredPrimitiveFixture")
  ...
  "StructWithOptionalInnerStructAndOptionalBooleanInside should round trip" should {
    val writeTarget =
      new FixedPathParquetScrooge[StructWithOptionalInnerStructAndOptionalBooleanInside](
        "/tmp/scalding_StructWithOptionalInnerStructAndBooleanInside")
    val readTarget = new FPLuiScroogeSource[StructWithOptionalInnerStructAndOptionalBooleanInside](
      "/tmp/scalding_StructWithOptionalInnerStructAndBooleanInside")

    val expected: List[StructWithOptionalInnerStructAndOptionalBooleanInside] = (0 until 100).map {
      i =>
        if (i % 3 == 1)
          StructWithOptionalInnerStructAndOptionalBooleanInside(None)
        else {
          if (i % 2 == 0) {
            StructWithOptionalInnerStructAndOptionalBooleanInside(
              Some(MaybeBoolean(i.toLong, None)))
          } else {
            StructWithOptionalInnerStructAndOptionalBooleanInside(
              Some(MaybeBoolean(i.toLong, Some(i % 3 == 0))))
          }
  ...
  "StructWithOptionalInnerStruct should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[StructWithOptionalInnerStruct](
      "/tmp/scalding_StructWithOptionalInnerStruct")
    val readTarget = new FPLuiScroogeSource[StructWithOptionalInnerStruct](
      "/tmp/scalding_StructWithOptionalInnerStruct")
  ...
  "PrimitiveMap should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[PrimitiveMap]("/tmp/scalding_PrimitiveMap")
    val readTarget = new FPLuiScroogeSource[PrimitiveMap]("/tmp/scalding_PrimitiveMap")

    val expected: List[PrimitiveMap] = (0 until 100).map { i =>
      val booleanMap = (0 until i + 2).map { idx =>
        (idx % 3 == 0) -> (idx % 3 == 0)
      }.toMap
      val byteMap = (0 until i + 2).map { idx =>
        idx.toByte -> idx.toByte
      }.toMap
      val shortMap = (0 until i + 2).map { idx =>
        idx.toShort -> idx.toShort
      }.toMap
      val integerMap = (0 until i + 2).map { idx =>
        idx -> idx
      }.toMap
      val longMap = (0 until i + 2).map { idx =>
        idx.toLong -> idx.toLong
      }.toMap
      val doubleMap = (0 until i + 2).map { idx =>
        idx.toDouble -> idx.toDouble
      }.toMap
      val stringMap = (0 until i + 2).map { idx =>
  ...
  "PrimitiveList should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[PrimitiveList]("/tmp/scalding_PrimitiveList")
    val readTarget = new FPLuiScroogeSource[PrimitiveList]("/tmp/scalding_PrimitiveList")

    val expected: List[PrimitiveList] = (0 until 100).map { i =>
      val booleanList = (0 until i + 2).map { idx =>
        (idx % 3 == 0)
      }
      val byteList = (0 until i + 2).map { idx =>
        idx.toByte
      }
      val shortList = (0 until i + 2).map { idx =>
        idx.toShort
      }
      val integerList = (0 until i + 2).map { idx =>
        idx
      }
      val longList = (0 until i + 2).map { idx =>
        idx.toLong
      }
      val doubleList = (0 until i + 2).map { idx =>
        idx.toDouble
      }
      val stringList = (0 until i + 2).map { idx =>
        idx.toString
      }
      PrimitiveList(booleanList,
                    byteList,
                    shortList,
                    integerList,
                    longList,
  ...
  "PrimitiveSet should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[PrimitiveSet]("/tmp/scalding_PrimitiveSet")
    val readTarget = new FPLuiScroogeSource[PrimitiveSet]("/tmp/scalding_PrimitiveSet")

    val expected: List[PrimitiveSet] = (0 until 100).map { i =>
      val booleanSet = (0 until i + 2).map { idx =>
        (idx % 3 == 0)
      }.toSet
      val byteSet = (0 until i + 2).map { idx =>
        idx.toByte
      }.toSet
      val shortSet = (0 until i + 2).map { idx =>
        idx.toShort
      }.toSet
      val integerSet = (0 until i + 2).map { idx =>
        idx
      }.toSet
      val longSet = (0 until i + 2).map { idx =>
        idx.toLong
      }.toSet
      val doubleSet = (0 until i + 2).map { idx =>
        idx.toDouble
      }.toSet
      val stringSet = (0 until i + 2).map { idx =>
        idx.toString
      }.toSet
  ...
  "Phone Source should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[Phone]("/tmp/scalding_phone")
    val readTarget = new FPLuiScroogeSource[Phone]("/tmp/scalding_phone")
  ...
  "AStructWithAStruct Source should round trip" should {
    val writeTarget =
      new FixedPathParquetScrooge[AStructWithAStruct]("/tmp/scalding_AStructWithAStruct")
    val readTarget = new FPLuiScroogeSource[AStructWithAStruct]("/tmp/scalding_AStructWithAStruct")
  ...
  "RequiredListFixture Source should round trip" should {
    val writeTarget =
      new FixedPathParquetScrooge[RequiredListFixture]("/tmp/scalding_RequiredListFixture")
    val readTarget =
      new FPLuiScroogeSource[RequiredListFixture]("/tmp/scalding_RequiredListFixture")

    val expected = (0 until 100).map { i =>
      val info = if (i % 2 == 0) Some(i.toString + "info") else None
      val lst = (0 until i + 2).map { idx =>
        Name("Major loop level: " + i.toString + ",minor:" + idx,
             Some("Major loop level: " + i.toString + "\t" + idx.toString))
      }
  ...
  "RequiredMapFixture Source should round trip" should {
    val writeTarget =
      new FixedPathParquetScrooge[RequiredMapFixture]("/tmp/scalding_RequiredMapFixture")
    val readTarget = new FPLuiScroogeSource[RequiredMapFixture]("/tmp/scalding_RequiredMapFixture")
  ...
  "RequiredEnumMapFixture Source should round trip" should {
    val writeTarget =
      new FixedPathParquetScrooge[RequiredEnumMapFixture]("/tmp/scalding_RequiredEnumMapFixture")
    val readTarget =
      new FPLuiScroogeSource[RequiredEnumMapFixture]("/tmp/scalding_RequiredEnumMapFixture")
  ...
  "TestFieldOfEnum Source should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[TestFieldOfEnum]("/tmp/scalding_TestFieldOfEnum")
    val readTarget = new FPLuiScroogeSource[TestFieldOfEnum]("/tmp/scalding_TestFieldOfEnum")
  ...
  "TestCaseNestedLists Source should round trip" should {
    val writeTarget =
      new FixedPathParquetScrooge[TestCaseNestedLists]("/tmp/scalding_TestCaseNestedLists")
    val readTarget =
      new FPLuiScroogeSource[TestCaseNestedLists]("/tmp/scalding_TestCaseNestedLists")
  ...
  "TestUnion Source should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[TestUnion]("/tmp/scalding_TestUnion")
    val readTarget = new FPLuiScroogeSource[TestUnion]("/tmp/scalding_TestUnion")
  ...
  "SimpleListList Source should round trip" should {
    val writeTarget = new FixedPathParquetScrooge[SimpleListList]("/tmp/scalding_SimpleListList")
    val readTarget = new FPLuiScroogeSource[SimpleListList]("/tmp/scalding_SimpleListList")
